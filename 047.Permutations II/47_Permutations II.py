...
def permuteUnique(nums: list[int]) -> list[list[int]]:
    #結果集
    result = []
    permuteUniqueDFS(nums, len(nums), [], result)
    return result

#nums為當前決策可以選擇的數值 numsLength為最外層初始nums的長度  path為已決策的陣列 滿足條件為result
def permuteUniqueDFS(nums: list[int], numsLength:int, path: list[int], result: list[list[int]]):
    #決策中止條件
    ...
    for index in range(len(new_List)):
        ...
        permuteUniqueDFS(numsCopy, numsLength, path + [new_List[index]], result)
